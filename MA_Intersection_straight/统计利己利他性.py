...
from irl.render import makeModel, render, get_dis, render_discrimination, render_discrimination_nogenerate
...
scenario_testfile_name = str(scenario_test) + 'west_left'
...
path_d = r'D:/Study/同济大学/博三/面向自动驾驶测试的仿真/sinD_nvn_xuguan/ATT-social-iniobs' \
               r'/MA_Intersection_straight/multi-agent-irl/irl/mack/multi-agent-trj/logger' \
               r'/airl/trj_intersection_4/decentralized/s-90/l-0.001-b-100-d-0.1-c-500-l2-0.1-iter-10-r-0.0/seed-13/'

scenario_name = "sample_trajss_" + str(scenario_testfile_name)
path_pkl = fr'D:\Study\同济大学\博三\面向自动驾驶测试的仿真\sinD_nvn_xuguan\ATT-social-iniobs\MA_Intersection_straight' \
       fr'\results_evaluate\s-90\有注意力的视频\专门生成生成轨迹的数据\{scenario_testfile_name}\{scenario_name}.pkl'  # path='/root/……/aus_openface.pkl'   pkl文件所在路径
f_generate = open(path_pkl, 'rb')
all_model_generate_trj = pickle.load(f_generate)
one_model_generate_trj = all_model_generate_trj[model_best][0]
sample_trajs = render_discrimination_nogenerate(path, path_d, model, env_id, mid, one_model_generate_trj, scenario_test)

# 指定文件夹路径
folder_path = fr'D:\Study\同济大学\博三\面向自动驾驶测试的仿真\sinD_nvn_xuguan\ATT-social-iniobs' \
              fr'\MA_Intersection_straight\results_evaluate\s-90\有注意力的视频\社会倾向\专门生成生成轨迹的数据\{scenario_testfile_name}'
# 将 scenario 加入文件夹路径中

# 创建文件夹（如果不存在）
os.makedirs(folder_path, exist_ok=True)
# scenario 名称
scenario_name = "sample_trajsspre重新得到的_" + str(scenario_test)
...
all_generate_trj = sample_trajs

# 根据sample_trajs绘制出利己性利他性变化视频
scenario_trj_ob = all_generate_trj[0]['ob']
scenario_trj_att_weight_spatial = all_generate_trj[0]['all_attention_weight_spatial']  # shape为（8, 230，21,10,10）
scenario_trj_ac = all_generate_trj[0]['ac']
scenario_trj_att_socialpre = all_generate_trj[0]['all_social']  # shape为（8, 230，1）
print('scenario_trj_ob:',type(scenario_trj_ob),len(scenario_trj_ob))
print('scenario_trj_att_weight:', type(scenario_trj_att_weight_spatial), len(scenario_trj_att_weight_spatial))
...
for left_i in range(3):
    ...
    left_weight_i_array = scenario_trj_att_weight_spatial[left_i]
    ...
    left_i_use = pd.concat([left_i_use_0, left_socialpre_i_use], axis=1)  # 横向拼接

    # 计算 socialpre_liji socialpre_lita列
    left_i_use['socialpre_liji'] = np.cos(left_socialpre_i_use)  # s-90版本训练反了，v5以及之后要调整过来
...
for straight_i in range(3,8):
    ...
    straight_weight_i_array = scenario_trj_att_weight_spatial[straight_i]
    ...
    straight_i_use = pd.concat([straight_i_use_0, straight_socialpre_i_use], axis=1)  # 横向拼接

    # 计算 socialpre_liji socialpre_lita列
    straight_i_use['socialpre_liji'] = np.cos(straight_socialpre_i_use)  # s-90版本训练反了，v5以及之后要调整过来
...
scenario_data_file_path = f'D:/Study/同济大学/博三/面向自动驾驶测试的仿真/sinD_nvn_xuguan/ATT-social-iniobs' \
                          f'/MA_Intersection_straight/results_evaluate/s-90/有注意力的视频/社会倾向' \
                          f'/专门生成生成轨迹的数据/{scenario_testfile_name}/scenario{scenario_test}_pre_data.csv'
...
def plot_top_view_ani_with_lidar_label(trj_in, frame_id_in, veh_id):
    ...
    trj_save_name = r'D:\Study\同济大学\博三\面向自动驾驶测试的仿真\sinD_nvn_xuguan\ATT-social-iniobs' \
                    r'\MA_Intersection_straight\results_evaluate\s-90\有注意力的视频\社会倾向\figure_save' \
...
def top_view_video_generation(start_frame_num, veh_id):
    ...
    for num in range(start_frame_num,
                     start_frame_num + len(os.listdir(r'D:\Study\同济大学\博三\面向自动驾驶测试的仿真\sinD_nvn_xuguan\ATT-social-iniobs'
                                                      r'\MA_Intersection_straight\results_evaluate\s-90'
        ...
        image_filename = r'D:\Study\同济大学\博三\面向自动驾驶测试的仿真\sinD_nvn_xuguan\ATT-social-iniobs' \
                         r'\MA_Intersection_straight\results_evaluate\s-90\有注意力的视频\社会倾向' \
    ...
    # 创建文件夹（如果不存在）
    video_save_path = fr'D:\Study\同济大学\博三\面向自动驾驶测试的仿真\sinD_nvn_xuguan\ATT-social-iniobs' \
                      fr'\MA_Intersection_straight\results_evaluate\s-90\有注意力的视频\社会倾向' \
                      fr'\figure_save\top_view_video\{scenario_testfile_name}'
    os.makedirs(video_save_path, exist_ok=True)

    video_save_name = fr'D:\Study\同济大学\博三\面向自动驾驶测试的仿真\sinD_nvn_xuguan\ATT-social-iniobs' \
                      fr'\MA_Intersection_straight\results_evaluate\s-90\有注意力的视频\社会倾向' \
                      fr'\figure_save\top_view_video\{scenario_testfile_name}\'_scenario_label_{veh_id}.avi'
