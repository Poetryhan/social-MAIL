...
# left_top = (-4.40, 34.46)
# left_bottom = (-4.40, -2.41)
# right_bottom = (33.46, -2.41)
# right_top = (33.46, 34.46)
# poly = geometry.Polygon([(5.10,34.46),
# (2.50,28.00),(-4.40, 25.80),
# (-4.40,7.00),
# (2.50,5.80),
# (5.10,-2.41),
# (21.80,-2.41),
...
experts = pkl.load(open(r'D:\Study\同济大学\博三\面向自动驾驶测试的仿真\sinD_nvn_xuguan'
                        ...
                        r'\AV_test\DATA\sinD_nvnxuguan_9jiaohu_social_dayu1_v2.pkl','rb'))
...
# experts = pkl.load(open(r'D:\Study\同济大学\博三\面向自动驾驶测试的仿真\sinD_nvn_xuguan\测试数据_东左\east_left_social_dayu1.pkl','rb'))
...
mids = ['0001'] + [str(a).rjust(4, '0') for a in range(50, 870, 50)] # + ['0260']  # 1001  (50, 1001, 50)
# mids = ['0001','0300']
print('mids:', len(mids), mids)
# mids: 15 ['0001', '0010', '0030', '0050', '0070', '0090', '0110', '0130', '0150', '0170', '0190', '0210', '0230', '0250', '0260']
# ['0001', '0050', '0100', '0150', '0200', '0250', '0300', '0350', '0400', '0450', '0500', '0550', '0600', '0650', '0700', '0750', '0800', '0850', '0900', '0950', '1000']

for scenario_test in [8]:
    results = []
    ...
    bleu_results = []
    sample_trajss = []
    print('evaluate_int_scenario_test:',scenario_test)
    # scenario_test = 6  #  6
    model = makeModel(env_id, scenario_test)
    scenario_test_name = str(scenario_test) + 'west_left'
    # mids=['0001']
    for iteration in range(len(mids)):
        mid = mids[iteration]
        path = r'D:\Study\同济大学\博三\面向自动驾驶测试的仿真\sinD_nvn_xuguan\ATT-social-iniobs' \
               r'\MA_Intersection_straight\multi-agent-irl\irl\mack\multi-agent-trj\logger' \
               r'\airl\trj_intersection_4\decentralized\s-90\l-0.001-b-100-d-0.1-c-500-l2-0.1-iter-10-r-0.0\seed-13\m_0' + mid
        # for i in range (450,451):
        # 在每次循环开始时清除默认图
        # tf.reset_default_graph()
        sample_trajs = render(path, model, env_id, mid, scenario_test)
    ...
    folder_path = r'D:\Study\同济大学\博三\面向自动驾驶测试的仿真\sinD_nvn_xuguan\ATT-social-iniobs' \
                  r'\MA_Intersection_straight\results_evaluate\s-90\有注意力的视频\专门生成生成轨迹的数据'
    # 将 scenario 加入文件夹路径中
    folder_path_with_scenario = os.path.join(folder_path, str(scenario_test_name))

    # 创建文件夹（如果不存在）
    os.makedirs(folder_path_with_scenario, exist_ok=True)
    # scenario 名称
    scenario_name = "sample_trajss_" + str(scenario_test_name)
    ...
    #                fr'\MA_Intersection_straight\results_evaluate\s-90\有注意力的视频\专门生成生成轨迹的数据' \
    #            fr'\{scenario_test_name}\sample_trajss_{scenario_test_name}.pkl'
    ...
    root = r'D:\Study\同济大学\博三\面向自动驾驶测试的仿真\sinD_nvn_xuguan\ATT-social-iniobs' \
           r'\MA_Intersection_straight\results_evaluate\s-90\训练集-评价结果'  # 评估9的时候写错了，写成了v4
    # 下面这一段代码是画出专家轨迹
    actionss = []
    expert = []
    ini_point = []
    ...
    for ii in range(scenario_test, scenario_test + 1):  # 原来是这个(107,108)，对应着第107个场景 len(experts) 112  测试场景
        ...
        plt.savefig(root + '\轨迹对比图片' + '\%s.png' % ('real_scenario_' + str(scenario_test_name) + ''), dpi=300)
    ...
    print('场景：',scenario_test,'有效的agent共有：',agent_num)
    # 专家的速度和加速度分布
    yaw_rate = [[], []]
    acc_rate = [[], []]
    ...
    generate_collide_situation_all = pd.DataFrame()  # 存放所有模型这个场景的生成轨迹的碰撞情况
    # for k in range(66):
    for ii in range(17):  # 模型迭代个数 21
        ...
        for i in range(1):  # 这个好像是和render，py文件中的num_trajs对应的，一个并行的模型跑两次，得到了两次结果,这里取1
            ...
            collide_situation = [traj_data["all_collide_situation"][j] for j in range(num_agents)]
            generate_collide_situation = pd.DataFrame(collide_situation).T # 存放一个模型这个场景的生成轨迹的碰撞情况
            print('collide_situation:', np.shape(collide_situation), np.shape(generate_collide_situation))
            generate_collide_situation.columns = ['agent0','agent1','agent2','agent3','agent4','agent5','agent6','agent7']
            generate_collide_situation['model_id'] = ii
            print('generate_collide_situation', generate_collide_situation)
            generate_collide_situation_all = pd.concat([generate_collide_situation_all,generate_collide_situation],axis = 0)
            ...
            for k in range(num_agents):
                ...
                if k not in sv and k < 3:
                    ...
                    plt.savefig(root + '\轨迹对比图片' + '\%s.png' % (
                                'scenario_' + str(scenario_test_name) + 'model_' + str(ii) + 'left_vehicle_' + str(k)), dpi=300)
                    plt.close()
            ...
            for k in range(num_agents):
                ...
                if k not in sv and k >= 3:
                    ...
                    plt.savefig(root + '\轨迹对比图片' + '\%s.png' % (
                                'scenario_' + str(scenario_test_name) + 'model_' + str(ii) + 'straight_vehicle_' + str(k)),
                                dpi=300)
                    plt.close()
            ...
            if use_lanelet2_lib:
                ...
                map_vis_without_lanelet.draw_map_without_lanelet(map_path, plt.gca(), 0, 0)
            plt.savefig(root + '\轨迹对比图片' + '\%s.png' % (
                        'scenario_' + str(scenario_test_name) + 'model_' + str(ii) + '_left_vehicle'), dpi=300)
            plt.close()
            ...
            plt.savefig(root + '\轨迹对比图片' + '\%s.png' % (
                        'scenario_' + str(scenario_test_name) + 'model_' + str(ii) + '_straight_vehicle'), dpi=300)
            ...
            plt.savefig(root + '\单个场景action分布图' + '\%s.png' % (str(scenario_test_name) + '_左转车_yaw_rate_' + 'tess_model_' + str(ii)))
            # plt.xlim(-28, 27)
            # plt.ylim(-4, 22)
            ...
            plt.savefig(root + '\单个场景action分布图' + '\%s.png' % (str(scenario_test_name) + '_直行车_yaw_rate_' + 'tess_model_' + str(ii)))

            fig4 = plt.figure(figsize=(6, 3 / 4 * 6))
            plt.hist(acc_rate[0], alpha=0.5)
            plt.hist(acc_rate_gerente[0], alpha=0.5)
            plt.savefig(root + '\单个场景action分布图' + '\%s.png' % (str(scenario_test_name) + '_左转车_acc_' + 'tess_model_' + str(ii)))

            fig5 = plt.figure(figsize=(6, 3 / 4 * 6))
            plt.hist(acc_rate[1], alpha=0.5)
            plt.hist(acc_rate_gerente[1], alpha=0.5)
            plt.savefig(root + '\单个场景action分布图' + '\%s.png' % (str(scenario_test_name) + '直行车_acc_' + 'tess_model_' + str(ii)))
    ...
    generate_expert_trj_all.to_csv(f"{root + '/用于做分布图和计算KL散度的数据'}/{scenario_test_name}生成轨迹和专家轨迹.csv")
    generate_expert_acc_yaw_all.to_csv(f"{root + '/加速度角度变化'}/{scenario_test_name}生成轨迹和专家轨迹的加速度和角度变化.csv")
    generate_collide_situation_all.to_csv(f"{root + '/碰撞情况'}/{scenario_test_name}生成轨迹碰撞情况.csv")
    ...
    # 创建一个新的索引
    average_pos_rmse_df_new_index = ['pos_left_rmse', 'pos_straight_rmse']

    # 设置新的索引并为其命名
    average_pos_rmse_df = average_pos_rmse_df.set_index(pd.Index(average_pos_rmse_df_new_index))
    ...
    average_pos_rmse_df.columns = ['model_0', 'model_1', 'model_2', 'model_3', 'model_4',
                                  ...
                                  'model_16'] # ,'model_17'] #,'model_18','model_19','model_20'] #,'model_21',
                                   # 'model_22','model_23','model_24','model_25','model_26',
                                   # 'model_27','model_28','model_29','model_30','model_31',
                                   # 'model_32','model_33','model_34','model_35','model_36','model_37','model_38']
    ...
    average_pos_rmse_df.to_csv(f"{root + '/轨迹位置的rmse'}/{scenario_test_name}生成轨迹和专家轨迹位置的rmse.csv")

    # 计算加速度和角度变化的RMSE
    generate_expert_acc_yaw_all = pd.read_csv(f"{root + '/加速度角度变化'}/{scenario_test_name}生成轨迹和专家轨迹的加速度和角度变化.csv")
    ...
    average_acc_yaw_df.columns = ['model_0', 'model_1', 'model_2', 'model_3', 'model_4',
                                  ...
                                  'model_16'] # ,'model_17'] # ,'model_18','model_19','model_20']#,'model_21',
                                   # 'model_22','model_23','model_24','model_25','model_26',
                                   # 'model_27','model_28','model_29','model_30','model_31',
                                   # 'model_32','model_33','model_34','model_35','model_36','model_37','model_38'] # ,'model_19','model_20'] #,'model_21','model_22','model_23','model_24','model_25','model_26']
                        # ]
        # ['model_0', 'model_1', 'model_2', 'model_3', 'model_4',
        #                           'model_5', 'model_6','model_7',
        #                           'model_8', 'model_9','model_10','model_11','model_12','model_13','model_14','model_15',
    ...
    average_acc_yaw_df.to_csv(f"{root + '/轨迹action的rmse'}/{scenario_test_name}生成轨迹和专家轨迹action的rmse.csv")


    # 统计场景中每个model_id的碰撞率
    model_datass_collide = [name[1] for name in generate_collide_situation_all.groupby(['model_id'])]  # 划分场景
    collide_rate = [[]]
    for one_model in model_datass_collide:
        columns = ['agent0','agent1','agent2','agent3','agent4','agent5','agent6','agent7']
        collide_agent_num = 0
        for one_agent in columns:
            collide_one_agent = one_model[one_agent]
            print('collide_one_agent:',collide_one_agent)
            if collide_one_agent.any():
                collide_agent_num = collide_agent_num + 1
            else:
                continue
        print('collide_agent_num:',collide_agent_num,'agent_num:',agent_num,'collide_agent_num/agent_num:',collide_agent_num/agent_num)
        collide_rate[0].append(collide_agent_num/agent_num)

    collide_rate_df = pd.DataFrame(collide_rate)
    print('collide_rate_df:',collide_rate_df)
    # 创建一个新的索引
    collide_rate_dfnew_index = ['collide_rate']

    # 设置新的索引并为其命名
    collide_rate_df = collide_rate_df.set_index(pd.Index(collide_rate_dfnew_index))
    # average_acc_yaw_df.columns = ['model_11']
    collide_rate_df.columns = ['model_0', 'model_1', 'model_2', 'model_3', 'model_4',
                                  'model_5', 'model_6','model_7','model_8', 'model_9','model_10','model_11',
                                  'model_12','model_13','model_14','model_15',
                                  'model_16']# ,'model_17'] # ,'model_18','model_19','model_20']
    collide_rate_df.to_csv(f"{root + '/生成轨迹碰撞率'}/{scenario_test_name}生成轨迹碰撞率.csv")
    ...
    def Jiaohu_change(agent_trj, neig_trj, type, time, direction):
        ...
        if type_jiaodian == 'point':
            ...
            return False, agent_trj_model_new.loc[0]
    # generate_expert_trj_all = pd.read_csv(r'E:\wsh-科研\nvn_xuguan_sind\sinD_nvn_xuguan\MA_Intersection_straight\results_evaluate\s-90\用于做分布图和计算KL散度的数据\85_生成轨迹和专家轨迹.csv')
                    ...
                    if len(potential_interaction_data) > 0:
                        ...
                        if len(interaction_data_left) > 0:
                            ...
                            if interaction_data_left['distance'].min() <= 15:  # 如果这和agent距离最近的车和agent的距离在15m之内
                                ...
                                GT_value = Cal_GT(time_trj, neig_left, 'generate')
                        ...
                        if len(interaction_data_right) > 0:
                            ...
                            if interaction_data_right['distance'].min() <= 15:  # 如果这和agent距离最近的车和agent的距离在15m之内
                                ...
                                # 计算trj和这个交互对象的GT
                                time_trj = trj[trj['time_ms'] == time_agent].copy()
                                GT_value = Cal_GT(time_trj, neig_right, 'generate')
                                trj['GT_left'][i] = GT_value
    ...
    data_all_model.to_csv(f"{root+ '/专家和生成轨迹desried_acc'}/{scenario_test_name}_生成和专家轨迹的期望加速度.csv")
