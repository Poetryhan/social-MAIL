    ...
    def run(self):
        ...
        for n in range(self.nsteps):
            ...
            for k in range(self.num_agents):
                ...
                mb_rewards[k].append(rewards[k])  # adddddddddddddd +true_rewards[k]
...
def learn(policy, expert, env, env_id, seed, total_timesteps=int(40e6), gamma=0.99, lam=0.95, log_interval=1, nprocs=32,
    ...
    elif disc_type == 'decentralized-2':
        discriminator = [
            Discriminator(model.sess, ob_space, ac_space,
                          state_only=True, discount=gamma, nstack=nstack, index=k, disc_type=disc_type,
                          scope="Discriminator_%d" % k, # gp_coef=gp_coef,
                          total_steps=total_timesteps // (nprocs * nsteps),
                          lr_rate=dis_lr, l2_loss_ratio=l2) for k in range(2)
        ]        
    else:
        assert False
    ...
    for update in range(1, total_timesteps // nbatch + 1):
        ...
        total_loss_2 = np.zeros((2, d_iters))
        ...
        for d_iter in range(d_iters):
            ...
            if disc_type == 'decentralized':
                ...
                g_obs_link = np.concatenate(g_obs[0:n_links], axis=1)
                g_actions_link = np.concatenate(g_actions[0:n_links], axis=1)
                g_nobs_link = np.concatenate(g_nobs[0:n_links], axis=1)
                e_obs_link = np.concatenate(e_obs[0:n_links], axis=1)
                e_actions_link = np.concatenate(e_actions[0:n_links], axis=1)
                e_nobs_link = np.concatenate(e_nobs[0:n_links], axis=1)
                
                g_obs_veh = np.concatenate(g_obs[n_links:num_agents], axis=1)
                g_actions_veh = np.concatenate(g_actions[n_links:num_agents], axis=1)
                g_nobs_veh = np.concatenate(g_nobs[n_links:num_agents], axis=1)
                e_obs_veh = np.concatenate(e_obs[n_links:num_agents], axis=1)
                e_actions_veh = np.concatenate(e_actions[n_links:num_agents], axis=1)
                e_nobs_veh = np.concatenate(e_nobs[n_links:num_agents], axis=1)
                
                total_loss[0, d_iter] = discriminator[0].train(
                        g_obs_link,
                        g_actions_link,
                        g_nobs_link,
                        g_log_prob[k].reshape([-1, 1]),
