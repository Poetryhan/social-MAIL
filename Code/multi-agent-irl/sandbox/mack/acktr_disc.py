    ...
    def __init__(self, policy, ob_space, ac_space, nenvs, total_timesteps, nprocs=2, nsteps=200,
        ...
        for k in range(num_agents):
            if identical[k]:
                ...
                A.append(tf.placeholder(tf.float32, [nbatch * scale[k],2]))
                ...
                print('identical[k]为：', identical[k],A[k])
        ...
        for k in range(num_agents):
            ...
            print('A[K]是：',A[k], 'train_model[k].pi是：',train_model[k].pi)
            # logpac = tf.nn.sparse_softmax_cross_entropy_with_logits(
            #     logits=train_model[k].pi, labels=A[k]) #源代码，报错Rank mismatch: Rank of labels (received 2) should equal rank of logits minus 1 (received 2)
            logpac = tf.nn.sparse_softmax_cross_entropy_with_logits(
                logits=train_model[k].pi, labels=tf.argmax(A[k], axis=1, output_type=tf.int32))
...
class Runner(object):
    def __init__(self, env, model, nsteps, nstack, gamma, lam):
        ...
        self.n_actions = [env.action_space[k].shape[0] for k in range(self.num_agents)]  # 原来是self.n_actions = [env.action_space[k].n for k in range(self.num_agents)]
...
def learn(policy, env, seed, total_timesteps=int(40e6), gamma=0.95, lam=0.92, log_interval=1, nprocs=32, nsteps=20,
    ...
    model = make_model() # 用于表示策略和价值函数，并用于训练和采样。

    runner = Runner(env, model, nsteps=nsteps, nstack=nstack, gamma=gamma, lam=lam)  # 创建一个 Runner 对象，它用于采样经验数据。
    nbatch = nenvs*nsteps
    tstart = time.time()
    coord = tf.train.Coordinator()
    # enqueue_threads = [q_runner.create_threads(model.sess, coord=coord, start=True) for q_runner in model.q_runner]
    for update in range(1, total_timesteps//nbatch+1):   # 进入一个循环，该循环用于训练模型。在每个更新步骤中，它运行 Runner 以获取经验数据，然后使用这些数据来更新策略和价值函数。训练过程中会记录一些性能指标，如策略损失、价值损失、策略熵等。
