        ...
        for agid, agent in enumerate(self._unwrapped.agents):
            if isinstance(agent.observation_space, spaces.Box):
                newobservation_space = spaces.Box(low=ent.observation_space.low[0],
                                                  ...
                                                  shape=self._buffer[agid].shape, dtype=np.float32)
...
class BipedalWalker(Agent):
    ...
    def observation_space(self):
        ...
        return spaces.Box(low=-np.inf, high=np.inf, shape=(24 + 4 + 3 + idx,), dtype=np.float32)
...
class MultiWalkerEnv(AbstractMAEnv, EzPickle):
    ...
    def __init__(self, n_walkers=2, position_noise=1e-3, angle_noise=1e-3, reward_mech='local',
                 ...
                 one_hot=False, competitive=False):
        EzPickle.__init__(self, n_walkers, position_noise, angle_noise, reward_mech, forward_reward,
                          fall_reward, drop_reward, terminate_on_fall, one_hot, competitive)
    ...
    @property
    def action_space(self):
        return [agent.action_space for agent in self.walkers]

    @property
    def observation_space(self):
        return [agent.observation_space for agent in self.walkers]
    ...
    def reset(self):
        ...
        #np.array([np.eye(5)[np.random.choice(5,4)] for _ in range(n_walkers)])
        #return self.step(np.array([(([1] + [0] * (self.slice-1)) * 4) for _ in range(self.n_walkers)]))[0]
        return self.step(np.array([0, 0, 0, 0] * self.n_walkers))[0]
    '''
    def reshape_action(self, multi_one_hot_actions):
        act_vec = np.reshape(multi_one_hot_actions, (self.n_walkers, 4, self.slice))
        act_cont = []
        for i in range(self.n_walkers):
            act_cont.append([-1.+1./self.slice*(2.*np.where(act_vec[i,j] == 1)[0][0]+1.) for j in range(4)])
        return act_cont
    '''
    def step(self, actions):
        #print (np.array(multi_one_hot_actions).shape)
        #act_vec = self.reshape_action(multi_one_hot_actions)
        #print (act_vec)
        ...
        for i in range(self.n_walkers):
            ...
            #         if k == i:
            #             shaping += 130 * np.abs(self.walkers[i].hull.position[0] - self.package.position.x - self.package_length/2)
            #         else:
            #             shaping -= 130 * np.abs(self.walkers[i].hull.position[1] - self.package.position.x - self.package_length/2)
        ...
        return obs, [np.mean(rewards) for _ in range(self.n)], [done for _ in range(self.n_walkers)], {}
...
if __name__ == "__main__":
    n_walkers = 2
    reward_mech = 'local'
    env = MultiWalkerEnv(n_walkers=n_walkers, reward_mech=reward_mech, competitive=False)
    print (env.action_space)
    print (env.observation_space)
    env.reset()
    print (env.agents[0].action_space.sample())
    for i in range(1000):
        env.render()
        #a = np.array([np.eye(5)[np.random.choice(5,4)] for _ in range(n_walkers)])
        #print (a.shape)
        a = np.array([env.action_space[0].sample() for _ in range(n_walkers)])
        o, r, done, _ = env.step(a)
        # print (len(o), o[0].shape)
        #print("\nStep:", i)
        #print "Obs:", o
        #print("Rewards:", r)
        #print "Term:", done
