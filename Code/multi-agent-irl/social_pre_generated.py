'''
Strategies SVO for Generated Scenarios
'''
import pandas as pd
from irl.render import makeModel, render, get_dis, render_discrimination_nogenerate
...
# generated_trj path
root_data = r'Code\results_evaluate\s-90\attention_video\trj_data'
scenario_name_already = "sample_trajss_" + str(scenario_testfile_name)
path_pkl = os.path.join(root_data, scenario_testfile_name, f"{scenario_name_already}.pkl")
...
sample_trajs = render_discrimination_nogenerate(path, path_d, model, env_id, mid, one_model_generate_trj, scenario_test, training_label)

# save path
root_SVO_save = r'Code\results_evaluate\s-90\attention_video\social_pre'
os.makedirs(root_SVO_save, exist_ok=True)

folder_path = os.path.join(root_SVO_save, 'trj_data', scenario_testfile_name)
os.makedirs(folder_path, exist_ok=True)

scenario_name = "sample_trajsspre_withSVO_" + str(scenario_test)
...
scenario_trj_att_socialpre = all_generate_trj[0]['all_social']
...
scenario_data_file_path = os.path.join(
    folder_path,
    f'scenario{scenario_test}_pre_data.csv'
)
...
def top_view_video_generation(start_frame_num, veh_id):
    # this function experts one top view video based on top view figures
    img_array = []
    # temp_top_view_figure path
